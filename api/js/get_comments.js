    ...
    for(let i=0; i < comments.length; i++){
        ...
        for(let j=0; j < commLike.length; j++){
            if(commLike[j]["commentID"] == comments[i]["ID"]){
                articolo +=`
                <svg id="${comments[i]["ID"]}" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-heart-fill text-dark svg-navbar" viewBox="0 0 16 16" role="img" aria-label="Like button">
        ...
        if(!r){
            articolo += `
                <svg id="${comments[i]["ID"]}" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-heart text-dark svg-navbar" viewBox="0 0 16 16" role="img" aria-label="Unlike button">
